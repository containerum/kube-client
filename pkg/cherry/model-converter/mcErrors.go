...
func ErrUnableToConvert(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Unable to convert resource", StatusHTTP: 400, ID: cherry.ErrID{SID: "model-converter", Kind: 0x1}, Details: []string(nil), Fields: cherry.Fields(nil)}
...
func ErrInternalError(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Internal error", StatusHTTP: 500, ID: cherry.ErrID{SID: "model-converter", Kind: 0x2}, Details: []string(nil), Fields: cherry.Fields(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
